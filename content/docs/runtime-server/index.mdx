...
const html = await renderToString(h("div", { class: "greeting" }, "Hello, world!"))
...
app.get("/page-a", async (req, res) => {
  const html = await renderToString(h(PageA, null))
  res.end(html)
})

app.get("/page-b", async (req, res) => {
  const html = await renderToString(h(PageB, null))
  res.end(html)
})
```
...
function RequestWrapper(props: { reqId: string; delay: number }) {
  ...
  return h(AsyncReader, { delay: props.delay })
}
...
const results = await Promise.all(
  Array.from({ length: 50 }, (_, i) =>
    renderToString(
      h(RequestWrapper, { reqId: `req-${i}`, delay: Math.random() * 20 })
    )
  )
)
...
const html = await renderToString(h(UserProfile, { userId: "123" }))
```
...
const html = await renderToString(
  h(Wrapper, null, h("span", null, "child content"))
)
...
const html = await renderToString(
  h(Layout, null, h("header", null, "Header"), h("main", null, "Content"))
)
```
...
const stream = renderToStream(
  h("div", null,
    h("h1", null, "Dashboard"),
    h(Suspense, {
      fallback: h("p", null, "Loading user..."),
      children: h(UserData, null),
    }),
    h("p", null, "Footer content"),
  )
)
```
...
const stream = renderToStream(
  h("div", null,
    h(Suspense, {
      fallback: h("p", null, "Loading A..."),
      children: h(SlowComponentA, null), // resolves in 200ms
    }),
    h(Suspense, {
      fallback: h("p", null, "Loading B..."),
      children: h(SlowComponentB, null), // resolves in 100ms
    }),
  )
)
```
...
function App() {
  ...
  return h(Suspense, {
    ...
    children: h(AsyncContent, null), // useContext(ThemeCtx) returns "dark"
  })
}
```
...
| `n-show` | Client-side visibility directive |
| `n-*` (custom directives) | All `n-`-prefixed directives are client-only |
...
```ts
h("div", { class: ["foo", null, "bar", false, "baz"] })
...
```ts
h("label", { className: "lbl", htmlFor: "inp" })
// => '<label class="lbl" for="inp"></label>'

h("div", { dataTestId: "val" })
...
h("img", { src: "data:text/html,<h1>hi</h1>" })
// => '<img />'  (src omitted)
```
...
```ts
h("br", null)     // => '<br />'
h("img", { src: "/pic.png" })  // => '<img src="/pic.png" />'
h("input", { type: "text" })   // => '<input type="text" />'
```
...
```ts
h("p", null, '<script>alert("xss")</script>')
...
export async function handler(req: Request): Promise<Response> {
  const appStream = renderToStream(h(App, null))
...
export async function handler(req: Request): Promise<Response> {
  ...
  const html = await runWithRequestContext(async () => {
    ...
    return renderToString(h(App, { router }))
  })
...
export async function handler(req: Request): Promise<Response> {
  const { html, head, htmlAttrs, bodyAttrs } = await renderWithHead(h(App, null))
...
| `configureStoreIsolation(setter)` | `(fn: (provider: () => Map) => void) => void` | Enable per-request store isolation for concurrent SSR |
