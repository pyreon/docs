...
const useCounter = defineStore('counter', () => {
  ...
  return { count, doubled, isPositive, increment, decrement, reset, setTo }
})
```
...
console.log(a.val())        // "first" — second setup never ran
```
...
a.count.set(42)
console.log(b.count()) // 42 — same signal instance
```

### Using in Components

Call the store hook inside a component's setup function. The returned signals integrate directly with Pyreon's reactive rendering:
...
const useNotifications = defineStore('notifications', () => {
  const { name } = useUser() // Use the user store

  const messages = signal<string[]>([])

  const greeting = computed(() => `Hello, ${name()}! You have ${messages().length} messages.`)
...
const useTodos = defineStore('todos', () => {
  ...
  async function fetchAll() {
    loading.set(true)
    try {
      const data = await request<typeof items extends () => infer T ? T : never>('/todos')
      items.set(data)
    } finally {
      loading.set(false)
    }
  ...
  async function add(text: string) {
    const todo = await request<{ id: number; text: string; done: boolean }>('/todos', {
      method: 'POST',
...
The store's return type is inferred automatically from the setup function:
...
Every store instance returned by `defineStore` is enhanced with `$`-prefixed methods for inspection, patching, subscriptions, and cleanup. These are added automatically — no extra setup needed.
...
store.$patch({ firstName: "Alice", lastName: "Smith" })

// Function form — receives signal references for direct manipulation
store.$patch((state) => {
  state.firstName.set("Alice")
  state.lastName.set("Smith")
})
```
...
Register global plugins that run when any store is first created. Plugins receive the store instance and its ID:
...
addStorePlugin(({ store, storeId }) => {
  store.$subscribe((mutation, state) => {
    console.log(`[${storeId}]`, mutation.type, mutation.events)
  })
})
...
```ts
type StorePlugin = (context: { store: any; storeId: string }) => void
```
...
describe('useCounter', () => {
  ...
  test('reset produces fresh state', () => {
    ...
    count.set(99)
    resetStore('counter')
    const { count: freshCount } = useCounter()
...
describe('useNotifications (depends on useUser)', () => {
  test('greeting includes user name', () => {
    ...
    const { greeting } = useNotifications()
...
describe('isolated registry tests', () => {
  ...
  test('custom provider isolates state', () => {
    ...
    expect(useStore().val()).toBe(0) // Fresh state in registry B

    setStoreRegistryProvider(() => registryA)
    expect(useStore().val()).toBe(10) // Preserved state in registry A
  })
})
```
...
```ts
function inspectStore<T extends Record<string, unknown>>(store: T): Record<string, unknown> {
  const snapshot: Record<string, unknown> = {}
  for (const [key, val] of Object.entries(store)) {
    if (typeof val === 'function' && val.length === 0) {
      try {
        snapshot[key] = val() // Read signal or computed
      } catch {
        snapshot[key] = '[action]'
      }
    } else {
...
// Usage:
const store = useCounter()
console.table(inspectStore(store))
// | key       | value    |
// |-----------|----------|
// | count     | 0        |
// | doubled   | 0        |
...
- **Returns** `() => T` -- A hook function that returns the singleton store state.
...
- **`plugin`** (`StorePlugin`) -- Function receiving `{ store, storeId }`.

### Enhanced Store Instance Methods

Every store returned by `defineStore` includes these `$`-prefixed methods:

| Method | Description |
|--------|-------------|
| `$id` | The store's unique identifier string |
| `$state` | Readonly snapshot of all signal values |
| `$patch(obj)` | Batch-set signals from an object |
| `$patch(fn)` | Batch-set signals via function receiving signal refs |
| `$subscribe(cb, opts?)` | Listen to state changes, returns unsubscribe |
| `$onAction(cb)` | Intercept action calls with after/error hooks, returns unsubscribe |
| `$reset()` | Reset all signals to initial values |
| `$dispose()` | Tear down the store and remove from registry |
