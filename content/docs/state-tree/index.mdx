...
```ts
function createPersistenceMiddleware(key: string) {
  return (call: ActionCall, next: (call: ActionCall) => unknown) => {
    const result = next(call)
    // Save snapshot after every action
    const snapshot = getSnapshot(call.path === '/' ? counter : counter)
...
All public functions that accept a model instance (`getSnapshot`, `applySnapshot`, `onPatch`, `addMiddleware`) validate their input and throw a descriptive error if called on a non-model-instance:
