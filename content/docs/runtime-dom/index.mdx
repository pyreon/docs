...
`@pyreon/runtime-dom` is Pyreon's browser renderer. It mounts VNode trees directly to the DOM using surgical signal-driven updates -- no virtual DOM diffing. It supports hydration, CSS transitions, template cloning, keep-alive caching, and custom directives.
...
const unmount = render(h(App, null), document.getElementById("app")!)
```
...
Apply props to a DOM element. Handles event listeners, reactive values, static attributes, classes, styles, innerHTML sanitization, and custom directives.
...
The `class` and `className` props are always set via `setAttribute("class", ...)`. Both static and reactive values are supported:
...
Directives are special props prefixed with `n-` that receive the element and an `addCleanup` callback. They provide a way to encapsulate imperative DOM logic:

```ts
import type { Directive } from "@pyreon/runtime-dom"

// Simple directive -- no cleanup needed
const nFocus: Directive = (el) => {
  el.focus()
}

// Directive with reactive value and cleanup
...
// Usage in JSX:
<input n-focus={nFocus} />
<div n-tooltip={nTooltip(() => label())}>Hover me</div>
```
...
#### Built-in n-show Directive

The `n-show` directive toggles `display` based on a reactive boolean:

```tsx
<div n-show={() => isVisible()}>
  This content is shown/hidden via display: none
</div>
```

Unlike conditional rendering, `n-show` keeps the element in the DOM and preserves all state. It simply toggles `el.style.display` between `""` and `"none"`.

#### Real-World Directive Examples

**Click outside directive:**
```ts
const nClickOutside = (handler: () => void): Directive =>
  (el, addCleanup) => {
    const listener = (e: MouseEvent) => {
      if (!el.contains(e.target as Node)) handler()
    }
    document.addEventListener("click", listener)
    addCleanup(() => document.removeEventListener("click", listener))
  }

// Usage:
<div n-clickOutside={nClickOutside(() => closeDropdown())}>
  <DropdownContent />
</div>
```
...
```ts
const nInView = (callback: (visible: boolean) => void): Directive =>
  (el, addCleanup) => {
...
```ts
const nAutoResize: Directive = (el, addCleanup) => {
  const resize = () => {
  ...
  resize()
  addCleanup(() => el.removeEventListener("input", resize))
}

// Usage:
<textarea n-autoResize={nAutoResize} />
```
...
function ExpensiveForm() {
  ...
  return () => (
    <div class="form-container" onScroll={(e) =>
      scrollPos.set((e.target as HTMLElement).scrollTop)
    }>
...
| Type | Description |
|------|-------------|
| `Directive` | `(el: HTMLElement, addCleanup: (fn: () => void) => void) => void` |
