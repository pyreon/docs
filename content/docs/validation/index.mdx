...
3. If `result.success` is `false`, maps each `ZodIssue` to a `ValidationIssue` by joining `issue.path` with `.` (e.g., `["address", "city"]` becomes `"address.city"`)
...
Valibot uses standalone functions rather than methods on schema objects. Because of this, the Valibot adapters require you to pass the `safeParse` or `safeParseAsync` function explicitly.
...
ArkType uses a function-call syntax for validation. The schema is called as a function, and if validation fails, it returns an `ArkErrors` array (which has a `summary` property to distinguish it from regular arrays).
...
4. If an error, maps each `ArkError` to a `ValidationIssue` by joining `error.path` with `.`
...
|---|---|---|
| `zodSchema(schema)` | `<TValues>(schema: ZodSchema<TValues>) => SchemaValidateFn<TValues>` | Create a form-level validator from a Zod object schema. Uses `safeParseAsync`. |
| `zodField(schema)` | `<T>(schema: ZodSchema<T>) => ValidateFn<T>` | Create a per-field validator from a Zod schema. Returns first error message. |
...
| Function | Signature | Description |
|---|---|---|
| `arktypeSchema(schema)` | `<TValues>(schema: ArkTypeSchema<TValues>) => SchemaValidateFn<TValues>` | Create a form-level validator from an ArkType schema. Synchronous. |
| `arktypeField(schema)` | `<T>(schema: ArkTypeSchema<T>) => ValidateFn<T>` | Create a per-field validator from an ArkType schema. Returns first error message. |
