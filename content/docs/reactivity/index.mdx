...
| `computed` | Create a derived computed value |
| `effect` | Create a reactive side effect |
| `renderEffect` | Lightweight effect for DOM bindings |
| `_bind` | Compiler-internal static-dep binding |
| `batch` | Batch multiple signal updates |
| `nextTick` | Wait for microtask flush |
| `watch` | Watch a source with old/new callback and cleanup |
| `cell` | Lightweight class-based reactive cell |
| `Cell` | Cell class constructor |
| `createStore` | Deep reactive proxy store |
| `isStore` | Check if a value is a store proxy |
| `reconcile` | Surgical diff into a store |
| `createResource` | Async reactive data fetcher |
| `createSelector` | O(1) equality selector |
| `effectScope` | Create a new EffectScope |
| `EffectScope` | EffectScope class |
| `getCurrentScope` | Get the active EffectScope |
| `setCurrentScope` | Set the active EffectScope |
| `runUntracked` | Run without tracking dependencies |
| `setErrorHandler` | Set the global effect error handler |
| `onSignalUpdate` | Register a signal write listener (debug) |
| `why` | Trace the next signal update (debug) |
| `inspectSignal` | Print signal state to console (debug) |
