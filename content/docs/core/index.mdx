...
| Export | Description |
|--------|-------------|
| `h` | Hyperscript function (JSX compile target) |
| `Fragment` | Fragment symbol for wrapper-less rendering |
| `EMPTY_PROPS` | Shared empty props sentinel |
| `defineComponent` | Identity wrapper marking a component |
| `onMount` | Register a mount callback (optionally returns cleanup) |
| `onUnmount` | Register an unmount callback |
| `onUpdate` | Register a post-update callback |
| `onErrorCaptured` | Register an error handler for child errors |
| `createContext` | Create a context with a default value |
| `useContext` | Read the nearest context value |
| `withContext` | Provide a context value during a function |
| `pushContext` / `popContext` | Low-level context stack manipulation (renderer) |
| `setContextStackProvider` | Override context stack for SSR isolation |
| `createRef` | Create a mutable DOM ref |
| `Show` | Conditional rendering with `when` accessor |
...
| `registerErrorHandler` | Register global error handler for telemetry |
| `reportError` | Report an error to registered handlers |
| `runWithHooks` | Run a component with lifecycle tracking (renderer internal) |
| `propagateError` | Walk error handlers (renderer internal) |
| `dispatchToErrorBoundary` | Dispatch to nearest error boundary (renderer internal) |
