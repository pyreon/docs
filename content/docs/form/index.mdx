...
| --- | --- | --- | --- |
| `initialValues` | `TValues` | (required) | Initial values for each field |
| `onSubmit` | `(values: TValues) => void \| Promise<void>` | (required) | Submit handler |
| `validators` | `Partial<{ [K in keyof TValues]: ValidateFn }>` | `undefined` | Per-field validators |
| `schema` | `SchemaValidateFn<TValues>` | `undefined` | Schema-level validator |
| `validateOn` | `'blur' \| 'change' \| 'submit'` | `'blur'` | When to validate |
| `debounceMs` | `number` | `undefined` | Debounce delay for validators |

**Returns `FormState<TValues>`:**

| Property / Method | Type | Description |
| --- | --- | --- |
| `fields` | `{ [K in keyof TValues]: FieldState<TValues[K]> }` | Individual field states |
| `isSubmitting` | `Signal<boolean>` | Whether onSubmit is running |
| `isValidating` | `Signal<boolean>` | Whether async validation is running |
| `isValid` | `Computed<boolean>` | Whether all fields are valid |
| `isDirty` | `Computed<boolean>` | Whether any field is dirty |
| `submitCount` | `Signal<number>` | Number of submit attempts |
| `submitError` | `Signal<unknown>` | Error from onSubmit |
| `values()` | `() => TValues` | Get all current values |
| `errors()` | `() => Partial<Record<keyof TValues, string \| undefined>>` | Get all current errors |
| `setFieldValue(field, value)` | `(field, value) => void` | Set a field value |
| `setFieldError(field, error)` | `(field, error) => void` | Set a field error |
| `setErrors(errors)` | `(errors) => void` | Set multiple errors |
| `clearErrors()` | `() => void` | Clear all errors |
| `resetField(field)` | `(field) => void` | Reset a single field |
| `register(field, opts?)` | `(field, opts?) => FieldRegisterProps` | Get input binding props |
| `handleSubmit(event?)` | `(event?) => Promise<void>` | Submit the form |
| `reset()` | `() => void` | Reset the entire form |
...
| Type | Description |
| --- | --- |
| `FormState<TValues>` | Full form state object returned by `useForm` |
...
| `FieldArrayItem<T>` | Item in a field array (`{ key: number, value: Signal<T> }`) |
