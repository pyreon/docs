...
Each pass is applied during a single AST walk using the TypeScript parser. The compiler uses positional string replacements (applied right-to-left) rather than full AST-to-code generation, keeping the output close to the original source and preserving source map compatibility.
...
The compiler is built on the TypeScript parser (`ts.createSourceFile`) for accurate JSX position tracking. It performs string-level replacements using character offsets, applied from right-to-left so earlier positions remain valid. This approach avoids the overhead of full AST-to-code generation and keeps the output close to the original source.
